...
@instance MonoidalCategory OneCobObject OneCob begin
    ...
    id(A::OneCobObject) = OneCobs.gid(A.nwires) 
...
@instance MonoidalCategory WrappedObjectWord WrappedOneCob begin
    ...
    id(A::WrappedObjectWord) = WrappedOneCob(A,A,OneCobs.gid(nwires(A)))
...
function rep(w::MorphismWord)
    ...
    mordict = Dict([(f::Symbol,
                     OneCobs.gmorvar(nwires(T.dom(f)),
                                    nwires(T.cod(f)),
                                    f)
                     )
...
#something like
#  R=Representation(T,MonoidalCategory,Dict([(X,WrappedObjectWord(X))]),Dict([(f,OneCobs.gid(1))]))
