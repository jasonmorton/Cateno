...
function replacesymbol(s::Symbol,n::Symbol,oc::OneCob)
#    oc2 = deepcopy(oc)
    innerports = [replacesymbol(s,n,ip) for ip in oc.innerports]
    outerports = replacesymbol(s,n,oc.outerports)
    loops = oc.loops[:]
    if s in loops
        ix = findfirst(loops,s) #assuming symbol occurs at most once
        loops[ix] = n
    end
    graph = replacesymbol(s,n,oc.graph)
    OneCob(graph,innerports,outerports,loops)
end
...
function disjoint_union(g,h)#::GenericAdjacencyList,h::GenericAdjacencyList)
    @assert isempty(intersect(Set([x.key for x in g.vertices]),
                              Set([x.key for x in h.vertices]))) "symbols which are keys in the two graphs must be disjoint"
...
function replacesymbol(s::Symbol, n::Symbol, g::GenericAdjacencyList)
    h = onecobgraph()
    for v in g.vertices
        u = add_vertex!(h, (s == v.key)? n : v.key)
        #check these are inserted in same order/w same index:
        @assert u.index == v.index "insertion order differs"
    end
    for (i, neighbors) in enumerate(g.adjlist)
        for v in neighbors
            # check if edge is already present; otherwise will add
            # multiple edges, two for each original edge
            if h.vertices[v.index] in out_neighbors(h.vertices[i], h) 
                nothing
            else
                add_edge!(h,h.vertices[i],h.vertices[v.index])
            end
        end
    end
    h
...
function gotimes(phi,psi)
    intersection = intersect(Set([x.key for x in phi.graph.vertices]),
                             Set([x.key for x in psi.graph.vertices]))
    if !isempty(intersection)
        # we have overlap in the gensymbols.  Make new ones.
        # if this works, we should make it efficient, and traverse once
        # with a "replacesymbols"
        for s in intersection
            psi=replacesymbol(s,gensym(),psi)
        end
    ...
    @assert phi.label==psi.label string("Label mismatch for OneCobs ",
                                         phi," ",psi,"\n")
